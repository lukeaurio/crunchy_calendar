...
class LiveCrunchyrollTests(unittest.TestCase):
    ...
    def test_previous_week_page_returns_real_releases(self):
        ...
        watched = filter_releases(enabled, ["Mushoku Tensei: Jobless Reincarnation"])
        watched_forecast = forecast_releases(watched, source_week, target_week)
        self.assertEqual({item.language for item in watched_forecast}, {"japanese", "english"})
        self.assertTrue(all(item.predicted for item in watched_forecast))
